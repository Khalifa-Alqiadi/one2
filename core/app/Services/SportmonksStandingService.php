...
class SportmonksStandingService
{
    ...
    public function syncSeasonStandings(int $seasonId, ?string $locale = null): array
    {
        ...
        $response = Http::timeout(30)
            ->get($url, [
                'api_token' => $token,
                'locale'    => $locale,
                'include'   => 'participant;details.type;form;group;rule;rule.type',
            ]);
    ...
    private function normalizeStandingRow(array $row, int $seasonId): array
    {
        ...
        $ruleTypeCode = strtolower((string) (
            data_get($row, 'rule.type.code')
            ?? data_get($row, 'rule.type.developer_name')
        ...
        return [
            ...
            'rule_id'                => data_get($row, 'rule.id'),
            'rule_name'              => $ruleName,
            'rule_type_id'           => data_get($row, 'rule.type.id'),
    ...
    private function normalizeStandingRowOLD(array $row, int $seasonId): array
    {
        $details = collect(data_get($row, 'details', []))
            ->filter(fn($x) => is_array($x))
            ->keyBy(function ($x) {
    ...
    public function syncSeasonStandingsOld(int $seasonId, ?string $locale = null): array
    {
        $token = config('services.SPORTMONKS_TOKEN');
        $locale = $locale ?: 'ar';
