<div class="tab-pane fade" id="t-standings">
    ...
    @php
        ...
        $resolveRuleCode = function ($row) {
            return strtolower((string) (data_get($row, 'rule.type.code') ?? (data_get($row, 'rule_type_code') ?? '')));
        };

        $resolveRuleLabel = function ($row) {
            return (string) (data_get($row, 'rule.name') ??
                (data_get($row, 'rule.type.name') ??
                    (data_get($row, 'rule_name') ?? (data_get($row, 'rule_type_name') ?? ''))));
        };

        $resolveZoneClass = function ($row) use ($resolveRuleCode) {
            $code = $resolveRuleCode($row);
            ...
            return match (true) {
                str_contains($code, 'champions') => 'zone-blue',
                str_contains($code, 'europa') => 'zone-orange',
                str_contains($code, 'conference') => 'zone-green',
                str_contains($code, 'relegation'),
                str_contains($code, 'relegated'),
                str_contains($code, 'drop')
                    => 'zone-red',
                default => 'zone-none',
            };
        };
    ...
    <div class="standings-wrap-v2">
        ...
        @forelse($groups as $g)
                    ...
                    <tbody>
                        @forelse($standings as $row)
                            @php
                                ...
                                $rowClass = $resolveZoneClass($row);
                            @endphp

                            <tr
                                class="standing-row {{ $rowClass }} {{ $teamId == $homeID ? 'active' : '' }}
            ...
            @php
                $legendItems = collect($standings)
                    ->map(function ($row) use ($resolveZoneClass, $resolveRuleLabel) {
                        $class = $resolveZoneClass($row);
                        $label = trim($resolveRuleLabel($row));

                        if ($class === 'zone-none' || $label === '') {
                            return null;
                        }
                    ...
                    ->filter()
                    ->unique(fn($item) => $item['class'] . '|' . $item['label'])
                    ->values();
            @endphp
            <div class="standings-legend-v2">
                <div class="row">
                    @if ($legendItems->isNotEmpty())
                        <div class="col-md-6">
                            <div class="">
                                <h5 class="mb-2">{{__('frontend.qualification_elimination')}}</h5>
                                @foreach ($legendItems as $item)
                                    <div class="legend-item-v2 mb-2">
                                        <span
                                            class="legend-bar-v2 {{ str_replace('zone-', 'legend-', $item['class']) }}"></span>
                    ...
                    <div class="col-md-6">
                        <ul class="m-0 p-0  ">
                            <h5 class="mb-2">{{__('frontend.last_5_matches')}}</h5>
                            <li class="d-flex mb-2">
                                <span class="dot-v2 W mx-1">W</span>
                                <span>{{ __('frontend.won') }}</span>
                            </li>
                            <li class="d-flex mb-2">
                                <span class="dot-v2 D mx-1">D</span>
                                <span>{{ __('frontend.draw') }}</span>
                            </li>
                            <li class="d-flex mb-2">
                                <span class="dot-v2 L mx-1">L</span>
                                <span>{{ __('frontend.loss') }}</span>
                            </li>
                        </ul>
                    </div>
                </div>
...
{{-- <div class="tab-pane fade" id="t-standings">
    ...
    @php
        ...
        $resolveZoneClass = function ($row) use ($stringValue, $locale) {
            $ruleTypeCode = strtolower(
                $stringValue(
                    data_get($row, 'rule_type_code') ??
                        (data_get($row, 'rule.type.code') ??
                            (data_get($row, 'rule.type.developer_name') ?? (data_get($row, 'rule.type.name') ?? ''))),
                    $locale ?? 'ar',
                ),
            );
            ...
            $ruleName = strtolower(
                $stringValue(
                    data_get($row, 'rule_name') ??
                        (data_get($row, 'rule.name') ??
                            (data_get($row, 'rule_type_name') ?? (data_get($row, 'rule.type.name') ?? ''))),
                    $locale ?? 'ar',
                ),
            );

            $haystack = trim($ruleTypeCode . ' ' . $ruleName);
            ...
            if (
                str_contains($haystack, 'champions-league') ||
                str_contains($haystack, 'uefa-champions-league') ||
                str_contains($haystack, 'champions league')
            ) {
                return 'zone-blue';
            }

            if (str_contains($haystack, 'europa-league') || str_contains($haystack, 'europa league')) {
                return 'zone-orange';
            }

            if (
                str_contains($haystack, 'conference-league') ||
                str_contains($haystack, 'uefa-conference-league') ||
                str_contains($haystack, 'conference league')
            ) {
                return 'zone-green';
            }
            ...
            if (
                str_contains($haystack, 'relegation') ||
                str_contains($haystack, 'relegated') ||
                str_contains($haystack, 'drop') ||
                str_contains($haystack, 'هبوط')
            ) {
                return 'zone-red';
            }
        ...
        $resolveLegendClass = function ($row) use ($resolveZoneClass) {
            $zoneClass = $resolveZoneClass($row);

            return match ($zoneClass) {
                'zone-blue' => 'legend-blue',
                'zone-orange' => 'legend-orange',
                'zone-green' => 'legend-green',
                'zone-red' => 'legend-red',
                default => null,
            };
        };
    ...
    <div class="standings-wrap-v2">
        ...
        @forelse($groups as $g)
            @php
                ...
                $legendItems = $rows
                    ->map(function ($row) use ($resolveLegendClass) {
                        $legendClass = $resolveLegendClass($row);

                        $label =
                            (string) (data_get($row, 'rule_name') ??
                                (data_get($row, 'rule.name') ??
                                    (data_get($row, 'rule_type_name') ?? (data_get($row, 'rule.type.name') ?? ''))));

                        if (!$legendClass || trim($label) === '') {
                            return null;
                        }
                    ...
                    <tbody>
                        @forelse($rows as $row)
                            @php
                                ...
                                $form = collect(explode(',', (string) data_get($row, 'form', '')))
                                    ->filter()
                            ...
                            <tr
                                class="standing-row {{ $rowClass }} {{ $teamId == $homeID ? 'active' : '' }} {{ $teamId == $awayID ? 'active' : '' }}">
                                <td class="club-col">
                                    <div class="club-cell-v2">
                                        <span class="rank-num-v2">{{ $pos }}</span>
            ...
            @if ($legendItems->isNotEmpty())
                <div class="standings-legend-v2">
                    <div class="legend-title-v2">{{ __('frontend.qualification') }}</div>

                    <div class="legend-items-v2">
                        @foreach ($legendItems as $item)
                            <div class="legend-item-v2">
                                <span class="legend-bar-v2 {{ $item['class'] }}"></span>
                                <span>{{ $item['label'] }}</span>
                            </div>
                        @endforeach
                    </div>
...
@push('after-styles')
    <style>
        ...
        .standing-row.zone-blue td:first-child::after {
            background: #3b82f6;
        }
